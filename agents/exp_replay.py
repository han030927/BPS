...
class ExperienceReplay(ContinualLearner):
    def __init__(self, model, opt, params):
        ...
        self.using_seperate_loss=params.using_seperate_loss
    ...
    def train_learner(self, x_train, y_train, **kwargs):
        ...
        for ep in range(self.epoch):
            for i, batch_data in enumerate(train_loader):
                ...
                if self.using_seperate_loss:
                    ...
                    if self.params.trick['kd_trick_star']:
                        loss = 1 / ((self.task_seen + 1) ** 0.5) * loss + \
                               (1 - 1 / ((self.task_seen + 1) ** 0.5)) * self.kd_manager.get_kd_loss(logits,
                                                                                                     batch_x)
                    ...
                    for j in range(self.mem_iters):
                        ...
                        if mem_x.size(0) > 0:
                            ...
                            if self.params.agent=='DER':
                                loss_mem=  0.3*F.mse_loss(mem_logits, mem_y)
                                loss+=loss_mem
                            else:
                    ...
                    for j in range(self.mem_iters):
                        ...
                        if mem_x.size(0) > 0:
                            ...
                            if self.params.agent!='DER':
                                ...
                                mem_logits= self.model.forward(mem_x)
                                loss+=F.mse_loss(mem_y, mem_logits)
                ...
                # update mem
                if self.params.agent=='DER':
                    self.buffer.update(batch_x, logits.detach())
                else:
