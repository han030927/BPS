...
import copy
from utils.loss import SupConLoss,SupLoss
import pickle
...
class ContinualLearner(torch.nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self, model, opt, params):
        ...
        self.lbl_inv_map = {}
        self.class_task_map = {}
        self.cur_task_test_loader=None
        self.test_loaders=[]
        self.scr_models=['ICARL', 'SCR', 'SCP', 'SSCR','DREAM_SSCR','FSR','AFSR','ASR','DDM','MVU','OFSR','MFSR','FSSCR']
    ...
    def criterion(self, logits, labels):
        ...
        elif self.params.agent in ['MFSR']:
            SC=SupLoss(temperature=self.params.temp,batch_size=self.params.batch+self.params.eps_mem_batch)
            return SC(logits, labels)
        elif self.params.agent in self.scr_models:
    ...
    def evaluate(self, test_loaders):
        ...
        if self.params.trick['ncm_trick'] or self.params.agent in self.scr_models:
            exemplar_means = {}
                ...
                for i, (batch_x, batch_y) in enumerate(test_loader):
                    ...
                    if self.params.trick['ncm_trick'] or self.params.agent in self.scr_models:
                        ...
                        if self.params.agent == 'ER_DVC':
                            ...
                            logits = self.model.forward(batch_x)
                            # if self.params.agent == 'DDDER':
                            #     logits+= self.fast_model.forward(batch_x)
                            _, pred_label = torch.max(logits, 1)
                            correct_cnt = (pred_label == batch_y).sum().item()/batch_y.size(0)
