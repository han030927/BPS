...
from models.resnet import WideResNet18
...
class SummarizeUpdate(object):
    ...
    def new_task(self, num_classes, labels):
        ...
        self.optim_flag = True
        log_dir = "logs/summarize/task" + str(
            num_classes // self.params.num_classes_per_task) + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
       # self.writer = SummaryWriter(log_dir)
        self.global_step = 0

    def new_network(self,num_classes):
        if self.params.data == 'mini_imagenet':
            im_size = (84, 84)
        elif self.params.data == 'tiny_imagenet':
            im_size = (64, 64)
    ...
    def update(self, buffer, x, y, aff_x=None, aff_y=None, update_index=-1, transform=None, **kwargs):
        ...
        #     self.new_network(buffer.num_classes)
        ...
        if condense_flag:
            ...
            for cls_idx, c in enumerate(labelset):
                ...
                loss,ps_loss = self.match_loss(img_real, img_syn, lab_real, lab_syn, buffer)
                avg_loss += loss.item()
            ...
            # self.writer.add_scalar('Loss/total_dd_loss', avg_loss / len(labelset), self.global_step)
            # self.writer.add_scalar('Loss/past_assist_loss', avg_ps_loss/ len(labelset), self.global_step)
            # self.writer.add_scalar('Loss/intra_task_matching_loss', acg_task_loss.item() / len(labelset),
            #                        self.global_step)
            self.global_step += 1
    ...
    def match_loss(self, img_real, img_syn, lab_real, lab_syn, buffer):
        loss = maybe_cuda(torch.tensor(0.),self.params.cuda)
        ...
        if len(img_mem) > 0:
            ...
            mem_loss = maybe_cuda(torch.tensor(0.),self.params.cuda)

        # options of feature distribution matching and gradient matching
        if 'feat' in self.params.match:
            m3d_criterion=M3DLoss('gaussian')
            loss+=m3d_criterion(feat_syn,feat_real)
