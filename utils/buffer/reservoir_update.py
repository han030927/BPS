...
class Reservoir_update(object):
    ...
    def update(self, buffer, x, y, **kwargs):
        ...
        if place_left:
            ...
            if buffer.params.update == 'der':
                buffer.buffer_logits[buffer.current_index: buffer.current_index + offset].data.copy_(y[:offset])
            else:
        ...
        #select random indice, the length is equal to batch size
        ...
        if buffer.params.update=='der':
            buffer.buffer_logits[list(idx_map.keys())] = replace_y
        else:
