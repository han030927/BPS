...
class TinyImageNet(DatasetBase):
    ...
    def download_load(self):
        ...
        data_dir='./datasets/tiny-imagenet-200/train.pkl'
        data = pickle.load(open(data_dir, 'rb'))
        self.train_data = np.array(data['train']['data']).reshape((100000, 64, 64, 3))
        self.train_label = np.array(data['train']['target'])

        #test = pickle.load(open(test_dir, 'rb'))
        self.test_data = np.array(data['test']['data']).reshape((10000, 64, 64, 3))
        self.test_label = np.array(data['test']['target'])
