...
def multiple_run(params, store=False, save_path=None):
    # Set up data stream
    # log_path = os.path.join(params.logs_dir, params.data + '_' + params.tag, params.agent + '_' + str(params.mem_size), 'log.txt')
    # #my_logger=Logger(log_path)
   

    start = time.time()
    print('Setting up data stream')
    #my_logger.write('Setting up data stream\n')
    data_continuum = continuum(params.data, params.cl_type, params)
    data_end = time.time()
    print('data setup time: {}'.format(data_end - start))
    #my_logger.write('data setup time: {}\n'.format(data_end - start))
    if store:
    ...
    for run in range(params.num_runs):
        ...
        if params.online:
            for i, (x_train, y_train, labels) in enumerate(data_continuum):
                ...
                #my_logger.write("-----------run {} training batch {}-------------\n".format(run, i))
                print('size: {}, {}'.format(x_train.shape, y_train.shape))
                #my_logger.write('size: {}, {}\n'.format(x_train.shape, y_train.shape))
                agent.cur_task_test_loader=test_loaders[i]
                agent.test_loaders.append(agent.cur_task_test_loader)
            ...
            #my_logger.write('Training Start\n')
            ...
            #my_logger.write("----------run {} training-------------\n".format(run))
            print('size: {}, {}'.format(x_train_offline.shape, y_train_offline.shape))
            #my_logger.write('size: {}, {}\n'.format(x_train_offline.shape, y_train_offline.shape))
    ...
    if params.online:
        ...
        #my_logger.write('----------- Total {} run: {}s -----------\n'.format(params.num_runs, end - start))
        print('----------- Avg_End_Acc {} Avg_End_Fgt {} Avg_Acc {} Avg_Bwtp {} Avg_Fwt {}-----------'
              .format(avg_end_acc, avg_end_fgt, avg_acc, avg_bwtp, avg_fwt))
        #my_logger.write('----------- Avg_End_Acc {} Avg_End_Fgt {} Avg_Acc {} Avg_Bwtp {} Avg_Fwt {}-----------\n'
              #.format(avg_end_acc, avg_end_fgt, avg_acc, avg_bwtp, avg_fwt))
    else:
        print('----------- Total {} run: {}s -----------'.format(params.num_runs, end - start))
        #my_logger.write('----------- Total {} run: {}s -----------\n'.format(params.num_runs, end - start))
        t_coef = stats.t.ppf((1 + 0.95) / 2, params.num_runs - 1)
         # shape: (num_run, num_task)
        avg_acc_per_run = np.mean(accuracy_array, axis=1)  # mean of end task accuracies per run
        avg_end_acc = (np.mean(avg_acc_per_run), t_coef * sem(avg_acc_per_run))
        print("avg_end_acc {}".format(avg_end_acc))
        #my_logger.write("avg_end_acc {}\n".format(np.mean(accuracy_list)))
