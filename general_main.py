...
def main(args):
    ...
    log_path = os.path.join(
        args.logs_dir,
        f"{args.data}{args.tag}",
        args.agent,
        f"task_number_{args.num_tasks}_{args.seed}",
        f"{args.update}batch_size{args.batch}_epoch{args.epoch}_{args.mem_size}",
        f"m_{int(args.momentum_m * 100)}_{args.match}_consistency{args.consistency_type}.txt"
    )
...
if __name__ == "__main__":
    ...
    ########################Agent#########################
    parser.add_argument('--agent', dest='agent', default='BPS',
                        choices=['ER','ER_DVC', 'EWC', 'AGEM', 'CNDPM', 'LWF', 'ICARL', 'GDUMB', 'ASER', 'SCR', 'SSCR',
                                 'IID_offline','MIR','GSS',
                                 'BPS'],
    ...
    parser.add_argument('--retrieve', dest='retrieve', default='random',
                        choices=['MIR', 'random', 'ASER', 'match', 'mem_match','MGI','der'],
    ...
    ########################Ours########################
    parser.add_argument('--momentum_m',type=float,default=0.9985)
    parser.add_argument('--mem_iters', dest='mem_iters', default=1, type=int,
                        help='mem_iters')
    parser.add_argument('--distill_interval',type=int,default=1)
    parser.add_argument('--batch_cons_weight',type=float,default=0.5)
    parser.add_argument('--buffer_cons_weight',type=float,default=0.1)

    parser.add_argument('--save_pth',type=boolean_string, default=True)
    parser.add_argument('--consistency_type',type=str, default='mse',choices=['cos','l1','kl','mse'])


    ########################DER########################
    parser.add_argument('--der_alpha',default=0.2,type=float)
    parser.add_argument('--der_beta',default=0.5,type=float)
    parser.add_argument('--tensorboard',default=False,type=boolean_string)
    ########################MTTSCR########################
    parser.add_argument('--max_start_expert',default=100)
    parser.add_argument('--expert_saving_step',default=200)
    parser.add_argument('--expert_start_step',default=0,type=int)
    parser.add_argument('--syn_steps',default=10)
    ...
    parser.add_argument('--match', default='grad', type=str, choices=['grad', 'feat', 'grad_feat','grad_CE'],
    ...
    ########################ER#########################
    parser.add_argument('--mem_size', dest='mem_size', default=100,
                        type=int,
    ...
    parser.add_argument('--using_seperate_loss', dest='using_seperate_loss', default=True,
                        type=bool)
    ...
    ########################GSS#########################
    parser.add_argument('--gss_mem_strength', dest='gss_mem_strength', default=20, type=int,
    ...
    parser.add_argument('--n_smp_cls', dest='n_smp_cls', default=1.5,
                        type=float,
    ...
    ####################DVC######################
    parser.add_argument('--dl_weight', type=float, default=4.0,
                        help='dl_weight')
